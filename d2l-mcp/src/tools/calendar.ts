...
import { normalizeDate } from '../utils/marshal.js';
...
export const calendarTools = {
  get_upcoming_due_dates: {
    ...
    handler: async (args: { orgUnitId?: number; daysBack?: number; daysAhead?: number }): Promise<string> => {
      ...
      const now = new Date();
      const startDate = normalizeDate(new Date(now.getTime() - daysBack * 24 * 60 * 60 * 1000).toISOString()) || '';
      const endDate = normalizeDate(new Date(now.getTime() + daysAhead * 24 * 60 * 60 * 1000).toISOString()) || '';
